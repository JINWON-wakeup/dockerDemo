...
%>    
	<%@ include file="../include/header.jsp" %>
		...
		<c:forEach var="dto" items="${list}">
			<tr>                                                                                                                                                          
				<td>${dto.uno}</td>                                                                
	<%-- 			<td><a href="<c:url value='memberInfo.do?&searchType=${pDto.searchType}&keyword=${pDto.keyword}&cntPerPage=${pDto.cntPerPage}uno=${dto.uno}'/>" >${dto.id}</a></td> --%>
	    ...
	    <%-- <ul class="pagination justify-content-center">
	        <li class="page-item ${pDto.prevPage <=0 ? 'disabled':''}">
	            <a class="page-link" href="memberList.do?
	            &searchType=${pDto.searchType}&keyword=${pDto.keyword}&cntPerPage=${pDto.cntPerPage}uno=${dto.uno}viewPage=${pDto.prevPage}">이전</a>
	        </li>
	
	        <c:forEach var="i" begin="${pDto.blockStart}" end="${pDto.blockEnd}">
	            <li class="page-item ${pDto.viewPage == i ? 'active':''}">
	                <a class="page-link" href="memberList.do?
	                &searchType=${pDto.searchType}&keyword=${pDto.keyword}&cntPerPage=${pDto.cntPerPage}uno=${dto.uno}viewPage=${i}">${i}</a>
	            </li>
	        </c:forEach>
	
	        <li class="page-item ${pDto.blockEnd >= pDto.totalPage ? 'disabled':''}">
	            <a class="page-link" href="memberList.do?
	            &searchType=${pDto.searchType}&keyword=${pDto.keyword}&cntPerPage=${pDto.cntPerPage}uno=${dto.uno}viewPage=${pDto.nextPage}">다음</a>
	        </li>
	    </ul> --%>
	    ...
	    <ul class="pagination justify-content-center">
	        ...
	        <li class="page-item ${pDto.blockEnd >= pDto.totalPage ? 'disabled':''}">
	            <a class="page-link" href="#" data-page="${pDto.prevPage}">다음</a>
	        </li>
	    </ul>
	    </div>
